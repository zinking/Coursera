function results = a3(wd_coefficient, n_hid, n_iters, learning_rate, momentum_multiplier, do_early_stopping, mini_batch_size)
  %a3(0, 10, 70, 0.002, 0.9, false, 4) 2.300287 0.900000
  %a3(0, 10, 70, 0.01, 0.9, false, 4) 2.284538  0.693444
  %a3(0, 10, 70, 0.05, 0.9, false, 4) 2.008179 0.724778
  %a3(0, 10, 70, 0.2, 0.9, false, 4) 1.097623 0.348889
  %a3(0, 10, 70, 1.0, 0.9, false, 4) 2.038473  0.755333
  %a3(0, 10, 70, 5.0, 0.9, false, 4) 2.302585 0.881000 
  %a3(0, 10, 70, 20.0, 0.9, false, 4)
  
  %a3(0, 10, 70, 0.002, 0, false, 4)2.304352  0.900000
  %a3(0, 10, 70, 0.01, 0, false, 4)2.302230   0.881222
  %a3(0, 10, 70, 0.05, 0, false, 4) 2.293294 0.872111
  %a3(0, 10, 70, 0.2, 0, false, 4) 2.229988 0.679333
  %a3(0, 10, 70, 1.0, 0, false, 4) 1.596059 0.584444
  %a3(0, 10, 70, 5.0, 0, false, 4) 2.302015  0.900000
  ...
  results = repmat([0,0,0],[3,1]);
  for data_i = 1:3,
    data = datas2{data_i};
    data_name = data_names{data_i};
    
    rloss = loss(model, data, wd_coefficient);
    r0loss = 0;
    fprintf('\nThe loss on the %s data is %f\n', data_name, rloss);
    if wd_coefficient~=0,
      r0loss = loss(model, data, 0);
      fprintf('The classification loss (i.e. without weight decay) on the %s data is %f\n', data_name, r0loss);
    end
    
    rclassp = classification_performance(model, data);
    fprintf('The classification error rate on the %s data is %f\n', data_name, rclassp);
    results(data_i,1) = rloss;
    results(data_i,2) = rclassp;
    results(data_i,3) = r0loss;
  end
end
...
function ret = loss(model, data, wd_coefficient)
  ...
  hid_input = model.input_to_hid * data.inputs; % input to the hidden units, 
  %i.e. before the logistic. size: <number of hidden units> by <number of data cases>
  hid_output = logistic(hid_input); % output of the hidden units, 
  %i.e. after the logistic. size: <number of hidden units> by <number of data cases>
...
function ret = d_loss_by_d_model(model, data, wd_coefficient)
  % model.input_to_hid is a matrix of size hu*256 <number of hidden units> by <number of inputs i.e. 256>
  % model.hid_to_class is a matrix of size 10*hu  <number of classes i.e. 10> by <number of hidden units>
  % data.inputs is a matrix of size ni*256 <number of inputs i.e. 256> by <number of data cases>. 
  % Each column describes a different data case. 
  % data.targets is a matrix of size 10*ni <number of classes i.e. 10> by <number of data cases>. 
  % Each column describes a different data case. It contains a one-of-N encoding of the class, 
  % i.e. one element in every column is 1 and the others are 0.
  ...
  % Right now, it just returns a lot of zeros, which is obviously not the correct output. 
  % Your job is to replace that by a correct computation.
  ret.input_to_hid = (model.hid_to_class' * (class_prob - data.targets) .* hid_output .* (1 - hid_output) * data.inputs') / I + wd_coefficient * model.input_to_hid;
  ret.hid_to_class =  (class_prob - data.targets) * hid_output' / I  + wd_coefficient * model.hid_to_class;
