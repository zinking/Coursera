  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    def recovered: Observable[Try[T]] = {
      Observable( observable =>{
    	  obs.subscribe(
	        value=> observable.onNext(Success(value)),
	        //error=> observable.onNext(Failure(error)),
	        { error: Throwable =>
...
//     val obm = obs.map( x => 
//        Observable.interval(totalSec seconds).map( _=>x ).take(1)  
//        ).flatten
//     obs.merge( obm )
