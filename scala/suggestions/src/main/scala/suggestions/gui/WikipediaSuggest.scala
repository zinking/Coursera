...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
//    val suggestions: Observable[Try[List[String]]] = searchTerms.map(x =>
//      	suggestionList.listData.toList.filter( y => y.contains(x) )
//      ).recovered
    
//    val suggestions: Observable[Try[List[String]]] = searchTerms.sanitized.map(x =>
//      	wikiSuggestResponseStream(x).filter(y => y.contains(x))
//      ).flatten.recovered
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(ss) => suggestionList.listData_=(ss)
      }
    }

    // TO IMPLEMENT
//    val selections: Observable[String] =  Observable( observable =>{
//    	 button subscribe {
//    	   case 
//    	 }
//      })//1 way
    val selections: Observable[String] =  button.clicks.filter( c =>{
      			suggestionList.selection.items.length > 0
    		}
        ).map( c => suggestionList.selection.items.head )

    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.map( s =>{
    	ObservableEx(wikipediaPage(s)).recovered
    }).flatten

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(txt) => editorpane.text_=(txt)
      }
    }
