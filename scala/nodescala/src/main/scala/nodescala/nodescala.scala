    ...
    Future.run() { ct =>
      ...
      async {
	    while (ct.nonCancelled) {
	      ...
	      //l.nextRequest.flatMap( arg =>{
	    //	  respond( arg._2, ct, handler(arg._1) )
	      //})
	      //l.nextRequest.continueWith((req,ex)=>{
	      //  respond(ex,ct,handler(req))
	      //})
	      //def f1(req:Request,ex:Exchange){
	      //  respond(ex,ct,handler(req))
	      //}
	      //l.nextRequest.continueWith(f1.)
//	      l.nextRequest.flatMap(args=>{
//	        val req = args._1
//	        val ex  = args._2
//	        async{
//	          respond(ex,ct,handler(req))
...
object NodeScala {
  ...
  trait Listener {
    ...
    def nextRequest(): Future[(Request, Exchange)] = {
       ...
       //val f1 = Future[(Request, Exchange)]( (Request, Exchange)=>{})
//       async{
//		 createContext{ ex:Exchange =>
