...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val clstPoints:GenSeq[(Point,GenSeq[Point])] =
    points.map{ pt=>
      val clst = findClosest(pt,means)
      (clst,GenSeq(pt))
    } ++
    means.map{ clst =>
      (clst,GenSeq[Point]())
    ...
    clstPoints.groupBy{ pair=>
      pair._1
    } map { case (clst,lstTupls)=>
      (clst, lstTupls.flatMap { kv => kv._2 })
    }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans.map{ oldpt=>
      findAverage(oldpt, classified.getOrElse(oldpt, GenSeq.empty))
    }
  }

  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans.zip(newMeans).foldLeft(0.0d){ case (p,pts)=>
      val (opt,npt) = pts
      val d = opt.squareDistance(npt)
      p+d
    } <= eta
  }
  ...
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    val nclst = classify(points,means)
    val newMeans = update(nclst, means)
